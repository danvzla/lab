...
for ex in my_vars:
    print 'generate config file for device '+ex["host_name"]+' : conf_file_run_phase_'+ex["host_name"]+'.conf'
    conffile=open('conf_file_run_phase_'+ex["host_name"]+'.conf','w')
...
for ex in my_vars:
	...
	cfg.load(path='conf_file_run_phase_'+ex["host_name"]+'.conf', format='text')
	if cfg.commit() == True:
